	...
	<article>
		...
		<table>
			...
			<tbody>
				...
				<c:forEach var="bList" items="${list}" end="9">
					<tr>
						<td>${pageCnt}</td>
						<td><a
							href="boardView.do?bno=${bList.bno}&pageNum=${pageMaker.cri.pageNum }&amount=${pageMaker.cri.amount }">${bList.title }
								<span>(${bList.replycount})</span>
						</a></td>
						<td>${bList.writer }</td>
						<fmt:parseDate var="dateString" value="${bList.wdate }"
							pattern="yyyy-MM-dd" />
						<td><fmt:formatDate value="${dateString}"
								pattern="yyyy-MM-dd" /></td>
						<td>${bList.viewcount }</td>
						<td>${bList.answercount }</td>
					</tr>
